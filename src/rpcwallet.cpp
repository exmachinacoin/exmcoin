...
Value stakeforcharity(const Array &params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "stakeforcharity <EXMcoin address> <percent>\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // EXMcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <EXMcoinpubkey>\n"
            "Return information about <EXMcoinpubkey>.");
...
// EXMcoin: reserve balance from being staked for network protection
...
// EXMcoin: check wallet integrity
...
// EXMcoin: make a public-private key pair
...
Value cccustomchange(const Array& params, bool fHelp)
{
    ...
    if(!address.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid EXMcoin address");
...
Value multisend(const Array &params, bool fHelp)
{
	...
	if (fHelp || params.size() != 2)
        throw runtime_error(
			...
			"multisend <EXMcoin Address> <percent>\n"
