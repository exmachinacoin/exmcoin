...
// EXMcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//EXMcoin now iterates each utxo inside of CheckStakeKernelHash()
...
// EXMcoin: check 'spent' consistency between wallet and txindex
// EXMcoin: fix wallet spent state according to txindex
...
// EXMcoin: disable transaction (only for coinstake)
