...
// Copyright (c) 2015 The EXMcoin developers
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // EXMcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // EXMcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// EXMcoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // EXMcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // EXMcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // EXMcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // EXMcoin: track money supply and mint amount info
...
// EXMcoin: total coin age spent in transaction, in the unit of coin-days.
...
// EXMcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // EXMcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // EXMcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // EXMcoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // EXMcoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // EXMcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // EXMcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // EXMcoin: getblocks may not obtain the ancestor block rejected
    ...
    // EXMcoin: if responsible for sync-checkpoint send it
...
// EXMcoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EXMcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Thursday, 19-Oct-17 00:10:11 UTC";
        CTransaction txNew;
        txNew.nTime = 1508371811;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1508371811; //Friday, 24-Mar-17 02:44:04 UTC
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 12205140;
        if(fTestNet)
        {
            block.nNonce   = 12205140;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc306cae2dbbd29fa68707388ecdd95ec2e362bc36436842508b0df731300380b"));
        block.print();
        ...
        // EXMcoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // EXMcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // EXMcoin: tell downloading node about the latest block if it's
